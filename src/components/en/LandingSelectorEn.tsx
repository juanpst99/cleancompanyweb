...
const services = [
  { id: 'sofas', name: 'Sofas & Couches', icon: '🛋️' },
  { id: 'mattresses', name: 'Mattresses', icon: '🛏️' },
  { id: 'rugs', name: 'Rugs & Carpets', icon: '🧶' },
  { id: 'full360', name: 'Full 360° Package (Best Value)', icon: '⭐', highlight: true },
]

const cities = ['Medellín', 'Bogotá']

export default function LandingSelectorEn() {
  const whatsappNumber = useWhatsAppNumber()
  const [selectedService, setSelectedService] = useState('full360')
  ...
  const handleWhatsAppRedirect = () => {
    ...
    // Fire tracking + attribution capture before leaving the page
    const { ref } = trackWhatsAppClick('', '')

    const serviceName = services.find(s => s.id === selectedService)?.name

    // Message in English — removes friction for non-Spanish-speaking customers
    const message = `Hi Clean Company 👋 I'd like to get a quote for *${serviceName}* in *${selectedCity}*. I have photos and measurements ready. Can you help me with availability please?\n\n(Ref: ${ref})`
  ...
  return (
    <div className="w-full bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-6 sm:p-8 border border-gray-100 relative overflow-hidden">
      ...
      <div className="mb-6 mt-2">
        <h3 className="text-xl font-extrabold text-gray-900 mb-1">Get your quote in 2 minutes</h3>
        <p className="text-gray-500 text-sm">Select your service and city — we'll take care of the rest</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {services.map((srv) => (
          ...
          >
            ...
            {selectedService === srv.id && (
              <CheckCircle2 className="absolute bottom-3 right-3 w-5 h-5 text-blue-600" />
            )}
          </button>
        ))}
      </div>
      ...
      <div className="mb-8">
        <p className="font-semibold text-gray-900 mb-3 flex items-center text-sm">
          <MapPin className="w-4 h-4 mr-1 text-gray-500" /> Your city
        </p>
        ...
        {selectedCity === 'Medellín' ? (
          <p className="text-xs text-green-600 mt-2 font-medium flex items-center justify-center">
            ✓ Active service routes in El Poblado, Laureles, Envigado & surroundings.
          </p>
        ) : (
          <p className="text-xs text-green-600 mt-2 font-medium flex items-center justify-center">
            ✓ Priority dispatch routes active today in Bogotá.
          </p>
        )}
      </div>
      ...
      >
        <svg className="w-6 h-6 mr-2 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
